...
// this rule will be merged into hyp if you don't call the separate() method.
...
hyp.map(k.o).to(`open -a Obsidian`).desc('Obsidian').separate()
hyp.map(k.l).to(`open -a Line`).desc('Line').separate()
...
sW.map(k.p).to(`open -g 'rectangle://execute-action?name=right-half'`).desc('Right Half')
...
sS.map(k.j).to(k.volume_decrement).desc('Volume Down')
sS.map(k.p).to(k.play_or_pause).desc('Play/Pause')
sS.map(k.semicolon).to(k.fastforward).desc('Fast Forward')
...
const sC = hyp.layer(k.c).desc('Media Control').separate()
